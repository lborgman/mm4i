    ...
    async function doMakeGeneratedMindmap(strSourceLink, nodeArray, rootNode) {
        ...
        const addAIgeneratedMarker = () => {
            ...
            eltTellGenerated.addEventListener("click", async evt => {
                evt.stopImmediatePropagation();
                const isSaved = eltTellGenerated.classList.contains("saved");
                if (isSaved) {
                    // alert("already saved");
                    const eltCopy = eltTellGenerated.cloneNode(true);
                    eltCopy.id = "copy-generated-marker";
                    eltCopy.style.position = "unset";

                    // FIX-ME: fetch these?
                    eltCopy.style.backgroundColor = "green";
                    eltCopy.style.color = "white";
                    eltCopy.style.display = "flex";
                    eltCopy.style.gap = "10px";
                    eltCopy.style.alignItems = "center";
                    eltCopy.style.minHeight = "40px";
                    eltCopy.style.width = "fit-content";
                    eltCopy.style.padding = "8px";
                    eltCopy.style.borderRadius = "8px";
                    eltCopy.style.fontFamily = "Arial, Helvetica, sans-seric";

                    const body = mkElt("div", undefined, [
                        mkElt("p", undefined, eltCopy),
                        mkElt("p", undefined, `This marker is only a reminder that you have saved this AI-generated mindmap`),
                        mkElt("p", undefined, "Do you want to remove it?")
                    ])
                    const ans = await modMdc.mkMDCdialogConfirm(body, "Yes", "No");
                    console.log({ ans });
                    // debugger;
                    if (ans) eltTellGenerated.remove();
                    return;
                }
                const body = mkElt("div", undefined, [
                    mkElt("p", undefined, `This mindmap was generated by AI. Do you want to save it to your mindmaps?`)
                ])
                const ans = await modMdc.mkMDCdialogConfirm(body, "Yes", "No");
                if (!ans) return;
